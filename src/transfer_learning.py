...
def get_pretrained_model(model_name):
    ...
    models_path = os.path.join(config.PRETRAINED_WEIGHTS_PATH, config.BASE_MODEL.lower(), model_name.lower(), f'model_{model_name}_{config.BASE_MODEL}_765_final_weights.h5')
...
def define_output_folder_name(args):
    ...
    output_folder_name = f'{identification}{config.BASE_MODEL}_{args.normalization}_{str(args.quantification)}_TL_with_FT'
    if args.epochs == 0 or args.no_tuning:
        output_folder_name = f'{identification}{config.BASE_MODEL}_{args.normalization}_{str(args.quantification)}_TL_no_FT'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--csv-folds-dir', action='store', type=str, default=config.CSV_FOLDS_BASE_PATH, help="CSV com a divisão dos folds")
    ...
    for model_name in args.model:
        for fold in args.fold:    
            ...
            output_dir = os.path.join(config.OUTPUT_RESULTS_TRANSFER_LEARNING_PATH, output_folder_name, model_name, str(fold))
            os.makedirs(output_dir, exist_ok=True)

            output_results_file = os.path.join(output_dir, f"{config.TRANSFER_LEARNING_STRATEGY}_results.json")
            if os.path.exists(output_results_file):
                print(f'Ignoring model: {model_name} - Fold: {fold} - Pretrained: {config.BASE_MODEL}')
                continue
            

            # clear tf session to load a new models
            ...
            print(len(df_fold), len(df_fold[df_fold['set'] == 'train']))
            ...
            if tuning:
                print(f'Model: {model_name} - Fold: {fold} - Pretrained: {config.BASE_MODEL}')
                ...
                with open(os.path.join(output_dir, f"{config.TRANSFER_LEARNING_STRATEGY}_history.json"), "w") as f:
                    json.dump(history.history, f, default=str)

                del history

            output_weights_dir = os.path.join(config.OUTPUT_WEIGHTS_TRANSFER_LEARNING_PATH, output_folder_name, model_name, str(fold))
            ...
            del model
            print(f'Results saved in the file: {output_results_file}')
